...
function initialize() {
	...
	$('.button').each((index, item) => {
		...
		if (!$item.hasClass('is-liquid') && $item.is(':visible') && !$item.is(':disabled') && !$item.hasClass('button--disabled') && $svg.length !== 0 && $('html').hasClass('desktop')) {
			...
			const button = new LiquidButton($item.find('svg')[0]);
